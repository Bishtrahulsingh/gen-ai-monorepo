...
def combine_install_requirements(
    ...
    if any(ireq.comes_from is None for ireq in source_ireqs):
        # None indicates packages was directly specified.
        comes_from = None
    else:
...
class BaseResolver(metaclass=ABCMeta):
    ...
    def resolve(self, max_rounds: int) -> set[InstallRequirement]:
        r"""
        Find concrete packages versions for all the given InstallRequirements
...
class LegacyResolver(BaseResolver):
    ...
    def resolve(self, max_rounds: int = 10) -> set[InstallRequirement]:
        r"""
        Find concrete packages versions for all the given ``InstallRequirement``\ s
    ...
    def _group_constraints(
        ...
        packages.  For example, given the following constraints:

            Django<1.9,>=1.4.2
            django~=1.5
            Flask~=0.7

        This will be combined into a single entry per packages:
    ...
    def _resolve_one_round(self) -> tuple[bool, set[InstallRequirement]]:
        ...
        This is achieved by finding the best match for each packages
        in the repository and adding all requirements for those best
        packages versions.  Some of these constraints may be new
        or updated.
    ...
    def _iter_dependencies(
        ...
        # download and inspect the packages version and get dependencies
        # from there
