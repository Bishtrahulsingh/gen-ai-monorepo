...
def _prepare_requirements(
    ...
    for req in metadata.get_all("Requires-Dist") or []:
        ...
        if parts.requirement.name == package_name:
            # Replace packages name with packages directory in the requirement
            # string so that pip can find the packages as self-referential.
            # Note the string can contain extras, so we need to replace only
            # the packages name, not the whole string.
