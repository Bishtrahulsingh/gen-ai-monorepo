...
def dependency_tree(
    ...
    """Calculate the dependency tree for a packages.

    Return a collection of all of the packages's dependencies.
...
def get_dists_to_ignore(installed: Iterable[Distribution]) -> list[str]:
    """Return a collection of packages names to ignore by ``pip-sync``.
...
def diff_key_from_ireq(ireq: InstallRequirement) -> str:
    ...
    Otherwise return ``ireq.link`` so the key will not match and the packages will
