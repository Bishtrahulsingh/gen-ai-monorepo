...
def version_option(
    ...
    **kwargs: t.Any,
    ...
    version, so it must match the name of the installed packages.

    :param version: The version number to show. If not provided, Click
        will try to detect it.
    :param param_decls: One or more option names. Defaults to the single
        value ``"--version"``.
    :param package_name: The packages name to detect the version from. If
        not provided, Click will try to detect it.
    :param prog_name: The name of the CLI to show in the message. If not
        provided, it will be detected from the command.
    :param message: The message to show. The values ``%(prog)s``,
        ``%(packages)s``, and ``%(version)s`` are available. Defaults to
        ``"%(prog)s, version %(version)s"``.
    :param kwargs: Extra arguments are passed to :func:`option`.
    :raise RuntimeError: ``version`` could not be detected.

    .. versionchanged:: 8.0
        Add the ``package_name`` parameter, and the ``%(packages)s``
        value for messages.

    .. versionchanged:: 8.0
        Use :mod:`importlib.metadata` instead of ``pkg_resources``. The
        version is detected based on the packages name, not the entry
        point name. The Python packages name must match the installed
        packages name, or be passed with ``package_name=``.
    """
    if message is None:
    ...
    def callback(ctx: Context, param: Parameter, value: bool) -> None:
        ...
        echo(
            message % {"prog": prog_name, "packages": package_name, "version": version},
            color=ctx.color,
        )
        ctx.exit()
