...
class TestFindDistributions:
    ...
    def test_standalone_egg_directory(self, target_dir):
        ...
        dists = pkg_resources.find_distributions(str(target_dir))
        assert [dist.project_name for dist in dists] == ['my-test-packages']
    ...
    def test_zipped_sdist_one_level_removed(self, target_dir):
        shutil.copytree(
            TESTS_DATA_DIR / 'my-test-packages-zip', target_dir, dirs_exist_ok=True
        )
        dists = pkg_resources.find_distributions(
            str(target_dir / "my-test-packages.zip")
        )
        assert [dist.project_name for dist in dists] == ['my-test-packages']
        dists = pkg_resources.find_distributions(
            str(target_dir / "my-test-packages.zip"), only=True
        )
        assert not list(dists)
