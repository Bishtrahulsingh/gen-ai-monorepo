...
- "flat-layout": a Python project that does not use "src-layout" but instead
  have a directory under the project root for each packages::
...
class _Finder:
    """Base class that exposes functionality for module/packages finders"""
...
class PackageFinder(_Finder):
    ...
    def _find_iter(
        ...
        for root, dirs, files in os.walk(str(where), followlinks=True):
            ...
            for dir in all_dirs:
                ...
                # Should this packages be included?
    ...
    def _looks_like_package(path: StrPath, _package_name: str) -> bool:
        """Does a directory look like a packages?"""
...
class ConfigDiscovery:
    ...
    def _explicitly_specified(self, ignore_ext_modules: bool) -> bool:
        """``True`` if the user has specified some form of packages/module listing"""
    ...
    def _analyse_explicit_layout(self) -> bool:
        """The user can explicitly give a packages layout via ``package_dir``"""
    ...
    def _analyse_flat_layout(self) -> bool:
        ...
        This assumes that multi-packages dists are uncommon and refuse to support that
    ...
    def _ensure_no_accidental_inclusion(self, detected: list[str], kind: str):
        if len(detected) > 1:
            ...
            To find more information, look for "packages discovery" on setuptools docs.
            """
    ...
    def _find_name_single_package_or_module(self) -> str | None:
        """Exactly one module or packages"""
        for field in ('packages', 'py_modules'):
            items = getattr(self.dist, field, None) or []
            if items and len(items) == 1:
                log.debug(f"Single module/packages detected, name: {items[0]}")
                return items[0]

        return None

    def _find_name_from_packages(self) -> str | None:
        """Try to find the root packages that is not a PEP 420 namespace"""
        ...
        if parent_pkg:
            log.debug(f"Common parent packages detected, name: {parent_pkg}")
            return parent_pkg

        log.warn("No parent packages detected, impossible to derive `name`")
        return None
...
def find_parent_package(
    ...
    """Find the parent packages that is not a namespace."""
...
def find_package_path(
    ...
    """Given a packages name, return the path where it should be found on
