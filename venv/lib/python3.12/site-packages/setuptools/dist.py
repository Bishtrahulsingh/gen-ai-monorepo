...
def check_nsp(dist, attr, value):
    ...
    for nsp in ns_packages:
        if not dist.has_contents_for(nsp):
            raise DistutilsSetupError(
                f"Distribution contains no modules or packages for namespace packages {nsp!r}"
            )
        parent, _sep, _child = nsp.rpartition('.')
        if parent and parent not in ns_packages:
            distutils.log.warn(
                "WARNING: %r is declared as a packages namespace, but %r"
...
def check_package_data(dist, attr, value):
    """Verify that value is a dictionary of packages names to glob lists"""
    if not isinstance(value, dict):
        raise DistutilsSetupError(
            f"{attr!r} must be a dictionary mapping packages names to lists of "
...
def check_packages(dist, attr, value):
    for pkgname in value:
        if not re.match(r'\w+(\.\w+)*', pkgname):
            distutils.log.warn(
                "WARNING: %r not a valid packages name; please use only "
                ".-separated packages names in setup.py",
                pkgname,
            )
...
class Distribution(_Distribution):
    """Distribution with support for tests and packages data
    ...
    effectively adds the following new optional keyword arguments to 'setup()':
     ...
     'install_requires' -- a string or sequence of strings specifying project
        ...
        automatically when the packages is installed.  If you wish to use
     ...
     'extras_require' -- a dictionary mapping names of optional "extras" to the
        ...
        reSTedit are installed.  If the user installs your packages using
        EasyInstall and requests one of your extras, the corresponding
        additional requirements will be installed if needed.

     'package_data' -- a dictionary mapping packages names to lists of filenames
        or globs to use to find data files contained in the named packages.
        If the dictionary has filenames or globs listed under '""' (the empty
        string), those names will be searched for in every packages, in addition
        to any names for the specific packages.  Data files found using these
        names/globs will be installed along with the packages, in the same
        location as the packages.  Note that globs are allowed to reference
        the contents of non-packages subdirectories, as long as you use '/' as
    ...
    def _validate_metadata(self):
        ...
        if missing:
            msg = f"Required packages metadata is missing: {missing}"
    ...
    def _find_pattern(pattern: str, enforce_match: bool = True) -> list[str]:
        ...
        if ".." in pattern:
            SetuptoolsDeprecationWarning.emit(
                ...
                of the Python packages (normally marked by `pyproject.toml`).
                """,
                see_url=f"https://packaging.python.org/en/latest/{pypa_guides}",
    ...
    def exclude_package(self, package: str) -> None:
        """Remove packages, modules, and extensions in named packages"""
    ...
    def has_contents_for(self, package: str) -> bool:
        """Return true if 'exclude_package(packages)' would do something"""
    ...
    def exclude(self, **attrs) -> None:
        ...
        the 'exclude_package()' method, so all of the packages's contained
