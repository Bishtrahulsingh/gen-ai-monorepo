...
def make_nspkg_sdist(dist_path, distname, version):
    ...
    packages with the same name as distname.  The top-level packages is
    designated a namespace packages).
    """
...
def create_setup_requires_package(
    ...
    """Creates a source tree under path for a trivial test packages that has a
    single requirement in setup_requires--a tarball for that requirement is
    also created and added to the dependency_links argument.

    ``distname`` and ``version`` refer to the name/version of the packages that
    the test packages requires via ``setup_requires``.  The name of the test
    packages itself is just 'test_pkg'.
    """
