...
def create_example(path, pkg_root):
    ...
    if pkg_root == ".":
        ...
        # skip other files: flat-layout will raise error for multi-packages dist
    else:
...
def verify_example(config, path, pkg_root):
    ...
    if pkg_root == ".":
        # Auto-discovery will raise error for multi-packages dist
    ...
    assert expanded["tool"]["setuptools"]["include-packages-data"] is True
    assert "" in expanded["tool"]["setuptools"]["packages-data"]
    assert "*" not in expanded["tool"]["setuptools"]["packages-data"]
...
def test_discovered_package_dir_with_attr_directive_in_config(tmp_path, pkg_root, opts):
    ...
    config["tool"]["setuptools"].pop("packages-dir", None)
...
def test_include_package_data_by_default(tmp_path, config):
    """Builds with ``pyproject.toml`` should consider ``include-packages-data=True`` as
    default.
    """
    pyproject = tmp_path / "pyproject.toml"
    pyproject.write_text(config, encoding="utf-8")
    ...
    assert config["tool"]["setuptools"]["include-packages-data"] is True
