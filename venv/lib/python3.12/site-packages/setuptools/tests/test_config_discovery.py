...
class TestFindParentPackage:
    def test_single_package(self, tmp_path):
        # find_parent_package should find a non-namespace parent packages
...
class TestDiscoverPackagesAndPyModules:
    ...
    def test_purposefully_empty(self, tmp_path, config_file, param, circumstance):
        ...
        # When either parameter packages or py_modules is an empty list,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                # Just the top-level packages can have `-stubs`, ignore nested ones
...
class TestWithCExtension:
    ...
    def test_dont_skip_discovery_with_pyproject_metadata(self, tmp_path):
        ...
        the packages lists C-extensions, but does not configure py-modules or packages.

        This way we ensure users with complex packages layouts that would lead to the
...
class TestWithPackageData:
    ...
    def test_include_package_data(self, tmp_path, src_root, files):
        """
        Make sure auto-discovery does not affect packages include_package_data.
        See issue #3196.
        """
...
def test_preserve_explicit_name_with_dynamic_version(tmpdir_cwd, monkeypatch):
    ...
    files = {
        ...
        "pyproject.toml": DALS(
            """
            [project]
            name = "myproj"  # purposefully different from packages name
            dynamic = ["version"]
            [tool.setuptools.dynamic]
    ...
    def spy_analyse_name():
        ...
        # name remains instead of the packages name.
        orig_analyse_name()
