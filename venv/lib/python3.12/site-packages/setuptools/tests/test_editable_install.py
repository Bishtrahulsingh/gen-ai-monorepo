...
EXAMPLE = {
    'pyproject.toml': dedent(
        ...
        version = "3.14159"
        license = {text = "MIT"}
        description = "This is a Python packages"
        dynamic = ["readme"]
        classifiers = [
...
def test_editable_with_prefix(tmp_path, sample_project, editable_opts):
    ...
    # figure out where pip will likely install the packages
    site_packages_all = [
...
class TestLinkTree:
    ...
    FILES["pyproject.toml"] += dedent(
        """\
        [tool.setuptools]
        # Temporary workaround: both `include-packages-data` and `packages-data` configs
