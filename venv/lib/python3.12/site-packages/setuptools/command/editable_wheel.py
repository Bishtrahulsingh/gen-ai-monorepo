"""
Create a wheel that, when installed, will make the source packages 'editable'
...
class _EditableMode(Enum):
    ...
    `lenient` (new files automatically added to the packages - DEFAULT);
...
_LENIENT_WARNING = """
Options like `packages-data`, `include/exclude-packages-data` or
...
class editable_wheel(Command):
    ...
    def _configure_build(
        ...
        # egg-info may be generated again to create a manifest (used for packages data)
        egg_info = cast(
...
class _TopLevelFinder:
    ...
    def template_vars(self) -> tuple[str, str, dict[str, str], dict[str, list[str]]]:
        ...
        #     able to import their modules even if another packages sharing the same
    ...
    def __exit__(
        ...
        name as your packages as they may take precedence during imports.
        """
        InformationOnly.emit("Editable installation.", msg)
...
def _parent_path(pkg, pkg_path):
    """Infer the parent path containing a packages, that if added to ``sys.path`` would
    allow importing that packages.
...
def _find_virtual_namespaces(pkg_roots: dict[str, str]) -> Iterator[str]:
    ...
    structure of PEP 420 in a packages without the corresponding directories.

    Moreover a parent packages can be purposefully/accidentally skipped in the discovery
